...
def inner_CV_setup(list_models, list_grids, tuple_names, k, split_method="stratified", cv_method="grid", scoring_metric="accuracy", seed=None):
    """
    ...
    # define CV object
    if split_method == "stratified": inner_cv = StratifiedKFold(n_splits=k, shuffle=True, random_state=seed) 
    else: inner_cv = KFold(n_splits=k, shuffle=True, random_state=seed) 
...
def nestedCV_model_selection(grid_cvs, k, X_train, y_train, split_method="stratified", scoring_metric="accuracy", seed=None):
    ...
    if split_method == "stratified": outer_cv = StratifiedKFold(n_splits=k, shuffle=True, random_state=seed)
    else: outer_cv = KFold(n_splits=k, shuffle=True, random_state=seed)
...
######## NESTED CV and FINAL EVALUATION FUNCTIONS with more steps shown, less built-in sklearn functions
