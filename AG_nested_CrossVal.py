...
from sklearn.model_selection import StratifiedKFold, KFold, GridSearchCV, RandomizedSearchCV, cross_val_score, cross_validate, train_test_split
...
#################################################################################################
def inner_CV_setup(list_models, list_grids, tuple_names, k, split_method="stratified", cv_method="grid", scoring_metric="accuracy", rs=None):
    """
    ...
    - list_models = list of model objects to use, for example [KNeighborsClassifier(), LogisticRegression()]
    - list_grids = list of dictionaries of hyperparameters grids to use, for example [{'n_neighbors': [15, 30, 45]}, {'penalty': ['l2'],'C': [0.5, 1, 2]}]
    - tuple_names = tuple of strings with model names, for example ["kNN", "Logreg"]
    - k = number of splits for the inner cv
    - split_method = string with CV splits method ("stratified" or "simple")
    - cv_method = string with cross-validation technique ("grid" or "random")
    ...
    # define CV object
    if split_method == "stratified": inner_cv = StratifiedKFold(n_splits=k, shuffle=True, random_state=rs) 
    else: inner_cv = KFold(n_splits=k, shuffle=True, random_state=1) 
    for mod, grid, name in zip(list_models, list_grids, tuple_names):
        if cv_method == "grid": gcv = GridSearchCV(estimator=mod, param_grid=grid, scoring=scoring_metric, n_jobs=-1, cv=inner_cv, refit=True)   # set refit to True to used the best model in the outer loop to get score on the outer test set
        else: gcv = RandomizedSearchCV(estimator=mod, param_grid=grid, scoring=scoring_metric, n_jobs=-1, cv=inner_cv, refit=True)
        grid_cvs[name] = gcv
    return grid_cvs


def nestedCV_model_selection(grid_cvs, k, X_train, y_train, split_method="stratified", scoring_metric="accuracy", rs=None):
    """
    ...
    if split_method == "stratified": outer_cv = StratifiedKFold(n_splits=k, shuffle=True, random_state=rs)
    else: outer_cv = KFold(n_splits=k, shuffle=True, random_state=rs)
    for name, gs_est in sorted(grid_cvs.items()):   # loop trough the dictionary produced by the inner_CV_setup() function
        # use sklearn cross_validate function
...
def final_tuning(best_model, p_grid, k, X_train, y_train, split_method="stratified", cv_method="grid", scoring_metric="accuracy", rs=None):
    """
    ...
    """
    if split_method == "stratified": inner_cv = StratifiedKFold(n_splits=k, shuffle=True, random_state=rs) 
    else: inner_cv = KFold(n_splits=k, shuffle=True, random_state=rs) 
    if cv_method == "grid": gcv_model_select = GridSearchCV(estimator=best_model, param_grid=p_grid, scoring=scoring_metric, n_jobs=-1, cv=inner_cv, refit=True)
    else: gcv_model_select = RandomizedSearchCV(estimator=best_model, param_grid=p_grid, scoring=scoring_metric, n_jobs=-1, cv=inner_cv, refit=True)
    gcv_model_select.fit(X_train, y_train)
    final_best = gcv_model_select.best_estimator_
    return final_best


######## FULL NESTED CV INITIALIZATION + PROCESS ########
#### 0) Importing data, defining training and final evaluation test
...
#### 2) Building the pipelines (if needed, can be passed to GridsearchCV/RandomizedSearchCV instead of simple model)
#### 3) Setting up the parameters grids  (small to be fast)
param_grid1 = [{'n_neighbors': [15, 30, 45]}]
param_grid2 = [{'penalty': ['l2'],'C': [0.5, 1, 2]}]
...
best_mod = KNeighborsClassifier(n_neighbors=30)  # for example...
...
print("\n")
#### 7) Compute performance on final evaluation for any score
